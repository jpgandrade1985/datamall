...
with st.sidebar:
    ...
    start_month = st.selectbox('Mês Inicial', months, index=0, key=01)
    start_year = st.selectbox('Ano Inicial', years, index=0, key=-2)
    end_month = st.selectbox('Mês Final', months, index=len(months) - 1, key = 03)
    end_year = st.selectbox('Ano Final', years, index=len(years) - 1, key = 04)
